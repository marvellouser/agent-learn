...
function parseWebSearchToolInput(input: JsonValue | undefined): WebSearchToolInput | undefined {
...
function normalizeWebSearchOutput(output: JsonValue | WebSearchToolOutput): WebSearchToolOutput {
...
function parseWebSearchBlocks(text: WebSearchToolOutput): WebSearchResultItem[] {
  ...
  for (const block of blocks) {
    ...
    const summary = block.match(/摘要:\s*([\s\S]*?)(?=\n\s*网站名称:|$)/)?.[1]?.trim() ?? '';
      ...
      <div className="tool-panel__head">
        ...
        {query ? (
          <span className="tool-panel__query">
            「{query}」
            {count != null ? <span className="tool-panel__count"> · {count} 条</span> : null}
          </span>
        ) : null}
      </div>
      {items.length > 0 ? (
        <ul className="tool-panel__results">
          {items.map((it, idx) => (
            <li key={`${it.ref}-${idx}`} className="tool-panel__result">
              {it.ref ? <span className="tool-panel__ref">引用 {it.ref}</span> : null}
              {it.title ? <div className="tool-panel__title">{it.title}</div> : null}
              {it.url ? (
                <a className="tool-panel__url" href={it.url} target="_blank" rel="noreferrer">
                  {it.url}
                </a>
              ) : null}
              {it.summary ? <p className="tool-panel__summary">{it.summary}</p> : null}
              {it.siteName ? <span className="tool-panel__site">{it.siteName}</span> : null}
              {it.publishedAt ? <span className="tool-panel__time">{it.publishedAt}</span> : null}
            </li>
          ))}
        </ul>
      ) : (
...
function getPendingHint(name: string, inputJson: JsonValue | undefined): string | undefined {
  if (name === 'web_search') {
  ...
  return (
    ...
    >
      <div className="tool-panel__head tool-panel__head--send-mail">
        ...
        {subject ? <span className="tool-panel__mail-subject-hint">「{subject}」</span> : null}
      </div>
...
function DefaultToolOutput({ value }: { value: JsonValue | WebSearchToolOutput }) {
  const text =
    typeof value === 'string' ? formatDefaultToolString(value) : JSON.stringify(value, null, 2);
...
function ToolMessagePart({ part }: { part: AnyToolPart }) {
  ...
  if (part.state === 'output-error') {
    return <ToolErrorPanel name={name} message={part.errorText ?? '工具执行出错'} />;
  }
