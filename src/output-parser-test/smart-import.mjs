...
// 初始化模型（qwen3 等在 thinking 模式下禁止 tool_choice=指定函数，需关闭 thinking）
...
// 工具调用里顶层多是对象，用 items 包住数组（与模型实际返回一致）
const friendsExtractSchema = z
  .object({
    items: z.array(friendSchema).describe('好友信息列表'),
  })
  .describe('从文本中提取的好友');

const structuredModel = model.withStructuredOutput(friendsExtractSchema, {
...
async function extractAndInsert(text) {
  ...
  try {
    // 切换到 hello 数据库
    await connection.query(`USE hello;`); // 使用 AI 提取结构化信息
...
4. 返回格式必须是一个 JSON 数组，即使只有一个人也要放在数组中`;

    const extracted = await structuredModel.invoke(prompt);
    const results = extracted.items;
